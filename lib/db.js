...
// SECURITY: Enforce per-user limit to prevent storage exhaustion (CWE-770)
const MAX_CUSTOM_PACKING_ITEMS = 100;

function addCustomPackingItem(userId, category, itemText) {
  const count = getDb().prepare(
    'SELECT COUNT(*) as count FROM user_packing_items WHERE user_id = ? AND is_custom = 1'
  ).get(userId);
  if (count.count >= MAX_CUSTOM_PACKING_ITEMS) {
    throw new Error(`Maximal ${MAX_CUSTOM_PACKING_ITEMS} benutzerdefinierte Einträge erlaubt`);
  }
...
// SECURITY: Require userId to prevent unauthorized deletion of other users'
// push subscriptions (CWE-862: Missing Authorization)
function deletePushSubscription(endpoint, userId) {
  if (userId) {
    getDb().prepare('DELETE FROM push_subscriptions WHERE endpoint = ? AND user_id = ?').run(endpoint, userId);
  } else {
    // Fallback for internal cleanup (e.g., 410 Gone responses) — no user context
